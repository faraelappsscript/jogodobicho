// === SCRIPT COMPARTILHADO PARA MÚLTIPLAS PÁGINAS COM BANNER INTEGRADO ===
...
function captureAndStoreUrlParameter() {
  ...
  console.log('Script: Verificando parâmetros da URL:', window.location.search);
  console.log('Script: Link de referência encontrado:', referrerLink);
  ...
  if (referrerLink) {
    ...
    try {
      ...
      console.log('Script: Link de referência armazenado no localStorage:', referrerLink);
    } catch (error) {
      console.warn('Script: Link de referência inválido:', referrerLink);
    }
  } else {
    console.log('Script: Nenhum parâmetro de referência encontrado na URL');
  }
}
...
function getStoredReferrerLink() {
  ...
  console.log('Script: Recuperando link do localStorage:', link);
  return link;
}
...
function showBanner() {
    ...
    // Recuperar o link diretamente do localStorage (agora no mesmo escopo)
    const referrerLink = getStoredReferrerLink();
...
// Função principal de inicialização do banner
function initBanner() {
...
// Inicialização comum - ATUALIZADA COM CAPTURA DE PARÂMETRO E BANNER
function initializeCommonFeatures() {
    console.log('Script: Iniciando initializeCommonFeatures()');
    
    // Capturar e armazenar parâmetro de URL na inicialização
    captureAndStoreUrlParameter();
    
    // Inicializar o banner após um pequeno delay para garantir que o DOM esteja pronto
    setTimeout(() => {
        console.log('Script: Inicializando banner após delay');
        initBanner();
    }, 500);
...
// Exportar funções do banner para uso externo
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('Script: DOMContentLoaded disparado');
